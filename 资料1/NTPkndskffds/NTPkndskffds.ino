...
#define DHT11PIN D0          // SDA接D0  SCL 接地  引脚定义直接翻百度型号 DHT12
#define KEY       0          // 按键，GPIO0 NodeMCU Flash Key 按键在D3引脚低电平触发
...
const unsigned long NtpInterval = 1800*1000UL;   //NTP自动校时间隔(毫秒) 此处为半个小时校准一次
...
{0x21,0x41,0xFF,0x01,0x01,'z'},      // -1-  字符1
...
{0x55,'z'},                // -11- "-"日期分隔符
{0xC0,0xC0,0x3C,0x42,0x42,0x42,0x24,'z'},  // -12- "oC"温度符号
{0x00,0x00,'z'},                     // -13-  2空列
{0x06,0x06,'z'},                     // -14-  "."小数点
{0x01,0x00,'z'},                     // -15-  首列带点，用于NTP校时不成功时提醒
{0x32,0x34,0x08,0x16,0x26,'z'},      // -16-  百分号%
{0xFF,0x98,0x94,0x92,0x61,'z'},      // -17-  字母R
{0xFF,0x10,0x10,0x10,0xFF,'z'},      // -18-  字母H
{0x00,'z'},                     // -19-  1空列
};
...
unsigned long ntptime(IPAddress& address){
  ...
  const long timeZoneOffset = +28800;     //时区GMT +8  更改个位数可调校时误差
...
if(Rtctime.DayOfWeek()%10 == 1){
    putin_buf(11);
    putin_buf(1);
 }
 else if(Rtctime.DayOfWeek()%10 == 2){
     putin_buf(11);
    putin_buf(2);
 }
 else if(Rtctime.DayOfWeek()%10 == 3){
    putin_buf(11);
    putin_buf(3);
 }
 else if(Rtctime.DayOfWeek()%10 == 4){
    putin_buf(11);
    putin_buf(4); 
 }
 else if(Rtctime.DayOfWeek()%10 == 5){
    putin_buf(11);
    putin_buf(5); 
 }
 else if(Rtctime.DayOfWeek()%10 == 6){
    putin_buf(11);
    putin_buf(6); 
 }
 else{
...
void Load_Humidity(void){   //湿度组合与显示
 float Humidity = 0;
 if (rtcstatus == 0){
   int chk = DHT11.read(DHT11PIN);
  ...
  bn = 0; //初始化填充指针
  memcpy(disp_buf,clear_buf,32);    //清显示缓冲区
   putin_buf(19);    //在前面显示两列空格，让内容居中；
   if((Humidity > 99)|(Humidity < 0)){
    putin_buf(11);   //超出湿度范围，显示"--"；
    putin_buf(11);
     putin_buf(16); //显示湿度符号；
     putin_buf(17);
     putin_buf(18);
     ...
     putin_buf(int(Humidity*10)/100);     //显示湿度值（十位）
    putin_buf((int(Humidity*10)%100)/10);//显示湿度值(个位)
     putin_buf(16); //显示湿度符号；
     putin_buf(17);
     putin_buf(18);
...
void setup() {
  ...
  for(int address=0; address<devices; address++) {
      ...
      lc.setIntensity(address,0);   //设置LED亮度，0-15；
...
if (millis() / 1000 != lastdisp){   //主显示程序，循环显示时间、日期、温度
      ...
      else if(t <= 8){    //日期显示保持3秒
        Load_date();
      }
      else if(t <= 11){   //温度显示保持3秒
        Load_temp();
      }
       else if(t <= 13){   //温度显示保持3秒
        Load_Humidity();
      }
      else{
        t = 0;
